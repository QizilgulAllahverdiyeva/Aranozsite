...
const {data, setData, sortData,setSortData,titleInp , 
    setTitleInp,priceInp,setPriceInp,imgInp,setImgInp}=useContext(ProductsContext)
  ...
  const deleteData = async (id) => {
    await axios.delete(`http://localhost:5000/furnitures/${id}`)
    getAllData();
  }
  const {
    register,
  ...
  const createProduct = async (values)=>{
   await axios.post("http://localhost:5000/furnitures", {
      "title": titleInp,
      "price": priceInp,
      "img": imgInp
    })
        ...
        <TableBody>
          {sortData.map((row) => (
            ...
            >
              ...
              <TableCell align="center"><Button variant="contained" color="primary" onClick={() => deleteData(row._id)} >Delete</Button></TableCell>
